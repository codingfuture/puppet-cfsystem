...
module PuppetX::CfSystem
    ...
    def self.createTimer(options)
        timer_name = options[:service_name]
        ...
        unit_ini['Description'] ||= timer_name
        ...
        } unless content_ini.has_key? 'Install'

        # Timer
        #---
        content_ini['Timer'] = {} unless content_ini.has_key? 'Timer'
        timer_ini = content_ini['Timer']

        unless period.nil?
            timer_ini['OnBootSec'] = period
            timer_ini['OnUnitActiveSec'] = period
        end

        unless calendar.nil?
            timer_init['OnCalendar'] = calendar
        end
